  ...
  const handleThirdPartyLogin = async (provider: "apple" | "google") => {
    ...
    try {
      const { data, error } = await supabaseClient.auth.signInWithOAuth({
        provider,
        options: {
          redirectTo: `${window.location.origin}/dashboard`, // or some route
        },
      });
      ...
      if (error) {
        ...
        // The user is typically redirected to the provider's login screen automatically.
        // After a successful OAuth, Supabase will redirect them back to redirectTo.
        // data.url might be present if a manual redirect is needed:
        if (data?.url) {
